...
def run_producer():
    ...
    print(f"⏱️  Fréquence : 1 conteneur toutes les {INTERVAL_SECONDS} secondes.")
    print("-" * 50)
    ...
    for i in range(1, TOTAL_CONTAINERS + 1):
        ...
        try:
            # Envoi de la requête POST à l'API (comme le ferait le front-end ou un client Kafka)
        ...
        # Pause avant le prochain camion
...
if __name__ == "__main__":
    try:
        # Optionnel: Réinitialiser le yard avant de commencer si on veut un yard vide à chaque test
        print("🔄 Réinitialisation du yard...")
        requests.post(f"{API_URL}/yard/init", json={"blocks": 4, "rows": 10, "max_height": 4})
        time.sleep(1) # Petit temps d'arrêt pour laisser l'API respirer
